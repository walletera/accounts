...
func (app *App) Run(ctx context.Context) error {
	...
	err := app.configureMongoDB(ctx)
	if err != nil {
		return fmt.Errorf("failed configuring mongodb: %w", err)
	}
...
func (app *App) startPublicAPIHTTPServer(appLogger *slog.Logger) (*http.Server, error) {
	repository := mongodb.NewAccountsRepository(app.mongoClient, accountsDatabase, accountsCollection)
...
func (app *App) configureMongoDB(ctx context.Context) error {
	...
	err = createMongoDBUniqueIndexes(ctx, app)
	if err != nil {
		return fmt.Errorf("failed creating mongodb index: %w", err)
	}
